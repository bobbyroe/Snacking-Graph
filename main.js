...
var bar_height, d, height, hideInfoPanel, info_panel, is_sorted_by_alpha, left_pos, log, onClick, onKeyDown, panel, showInfoPanel, snack_data, sort_string, sort_strings, svg, toggleInfoPanel, type, update, width, y;
...
d3.csv("data/Snack_Composition.csv", type, function(error, data) {
  ...
  bar = svg.selectAll("g").data(snack_data).enter().append("g").on('click', function(d, i) {
    ...
    return "translate(" + left_pos + ", " + (y(d.Name)) + ")";
  });
  bar.append("rect").style('stroke-width', '1px').style('stroke', 'rgba(0, 98, 255, 0.8)').attr("width", function(data, i) {
  ...
  }).attr("y", bar_height - 9).text(function(d) {
...
update = function() {
  ...
  return transition.selectAll("g").delay(function(d, i) {
    ...
    return "translate(" + left_pos + ", " + (y0(d.Name)) + ")";
  });
};
